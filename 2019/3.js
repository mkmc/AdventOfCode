...
const lines1 = convertToLines(wire1)
const lines2 = convertToLines(wire2)
...
const lengths1 = new Array(intersections.length).fill(-1)
const lengths2 = new Array(intersections.length).fill(-1)
...
lines1.forEach(l => {
  intersections.forEach((ix, i) => {
    ...
    if (calcIsInsideLineSegment(l.start, l.end, ix) && calcDistancePointToLine(l.start, l.end, ix) < 1) {
      const dir = { x: ix.x - l.start.x, y: ix.y - l.start.y }
      const lastLength = Math.sqrt(dir.x * dir.x + dir.y * dir.y)
      lengths1[i] = currentLength + lastLength
    }
  })
  const dir = { x: l.end.x - l.start.x, y: l.end.y - l.start.y }
  currentLength += Math.sqrt(dir.x * dir.x + dir.y * dir.y)
})
...
lines2.forEach(l => {
  intersections.forEach((ix, i) => {
    ...
    if (calcIsInsideLineSegment(l.start, l.end, ix) && calcDistancePointToLine(l.start, l.end, ix) < 1) {
      const dir = { x: ix.x - l.start.x, y: ix.y - l.start.y }
      const lastLength = Math.sqrt(dir.x * dir.x + dir.y * dir.y)
      lengths2[i] = currentLength + lastLength
    }
  })
  const dir = { x: l.end.x - l.start.x, y: l.end.y - l.start.y }
  currentLength += Math.sqrt(dir.x * dir.x + dir.y * dir.y)
...
const totalLengths = new Array(intersections.length).fill(0).map((_, i) => lengths1[i] + lengths2[i])

console.log(Math.min(...totalLengths))
...
function convertToLines(wire) {
  const vectors = wire.split(',').map(code => {
    const length = parseInt(code.substring(1))
    ...
    switch (code[0]) {
      ...
      case 'D':
        ...
        break
      default:
        throw new Error('unknown command:', code[0])
    }
    return vector
  })
  ...
  vectors.forEach(vector => {
    const newPoint = { x: lastPoint.x + vector.x, y: lastPoint.y + vector.y }
    lines.push({ start: lastPoint, end: newPoint })
    lastPoint = newPoint
  })
